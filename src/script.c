...
char *buffer_pushkey(lua_State *L, char *start, int index) {
    char key[256];
    char *end = strchr(start, 0);
    snprintf(key, sizeof(key), "%s#%d", start, index);
...
void script_response(lua_State *L, int status, buffer *headers, buffer *body) {
    ...
    lua_newtable(L);
    for (char *c = headers->buffer; c < headers->cursor; i++) {
        c = buffer_pushkey(L, c, i);
        c = buffer_pushlstring(L, c);
        lua_rawset(L, -3);
    }
